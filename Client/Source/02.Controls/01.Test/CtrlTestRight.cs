...
namespace Hnc.Calorimeter.Client
{
    ...
    public partial class CtrlTestRight : UlUserControlEng
    {
        ...
        private void Initialize(int handle)
        {
            ...
            if (handle == 0) Resource.Variables = Context.Value;
        ...
        private void TestNormalTerminate()
        {
            TestTerminate();
        }

        private void TestAbnormalTerminate()
        {
            TestTerminate();
        }
        ...
        private void TestTerminate()
        {
            ...
            if (DefMenu.Index == 0) DoActivatedControls(null, null);
            else DefMenu.Index = 0;
            DefMenu.Enabled = false;
        ...
        private void startButton_Click(object sender, EventArgs e)
        {
            if (Context.IsTestThreadNull == true)
            {
                ...
                thread.InvalidGraph += meas.DoInvalidGraph;
                thread.SetToggleButtonState += DoSetToggleButtonState;
                thread.DispStateMainForm += Resource.MainForm.DoDispStateMainForm;
        ...
        {
            int handle = Context.Condition.Control.ScheduleGridView.FocusedRowHandle;
            if (handle < 0) return;

            OpenScheduleDialog(Context.Condition.Schedules[handle]);
        }
        ...
        private void openAllParamButton_Click(object sender, EventArgs e)
        {
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                if (form.RecNo >= 0)
                {
                    ...
                    try
                    {
                        LoadSchedule(form.RecNo);
                        LoadCondition(form.RecNo);
                    }
                    finally
                    {
                        Context.DB.Unlock();
        ...
        {
            Context.Condition.Control.IntegralCountEdit.Focus();
            ...
            try
            {
                form.UserNameEdit.Text = Context.DB.UserSet.GetName(Resource.UserNo);
            }
            finally
            {
                Context.DB.Unlock();
            ...
            form.DateTimeEdit.Text = DateTime.Now.ToString(Resource.csDateTimeFormat);
            form.MemoEdit.Text = "";
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                ...
                try
                {
                    Context.DB.AllParamSet.RecNo = (int)Context.DB.GetGenNo("GN_ALLPARAM");
                    Context.DB.AllParamSet.UserNo = Resource.UserNo;
                    Context.DB.AllParamSet.RegTime = form.DateTimeEdit.Text;
                    Context.DB.AllParamSet.Memo = form.MemoEdit.Text;
        ...
        public void DoActivatedControls(object sender, EventArgs e)
        {
            bool enabled = !Resource.BusySets[Context.Handle];

            openScheduleButton.Enabled  = enabled;
            openConditionButton.Enabled = enabled;
            openAllParamButton.Enabled  = enabled;
            saveAllParamButton.Enabled  = enabled;
            resetAllParamButton.Enabled = enabled;

            Context.Condition.Control.ActiveControls();
        }

        public void DoSetToggleButtonState(object sender, bool enabled)
        {
            if (this.InvokeRequired == true)
            {
                EventBoolHandler func = new EventBoolHandler(DoSetToggleButtonState);
                this.BeginInvoke(func, new object[] { sender, enabled });
            }
            else
            {
                DefMenu.Index = 1;
        ...
        {
            openScheduleButton_Click(null, null);
        }
        ...
        private void OpenConditionDialog()
        {
            ...
            if (form.ShowDialog() == DialogResult.OK)
            {
                if (form.RecNo >= 0)
                {
                    ...
                    try
                    {
                        Context.DB.NoteParamSet.SelectRecNo(form.RecNo);
        ...
        {
            Context.DB.BeginTrans();

            try
            {
                int paramNo = Context.DB.AllParamSet.RecNo;

                Context.DB.AllParamSet.Insert(Context.DB.Trans);
